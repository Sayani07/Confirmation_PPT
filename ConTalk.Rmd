...
# <span style="color:DarkBlue"> Across Day-of-Week
...
p1 = SU_uniform_mutate %>% filter(Source=="B1 05") %>%
  ...
  mutate(wday = wday(date, label = TRUE, abbr = TRUE,
                     week_start = 1)) %>%
  ggplot(aes(x=wday, y=dkwh)) + geom_boxplot() + ylab("Daily Consumpttion in KwH ") + 
...
---
##<span style="color:DarkBlue"> Across Hour-of-Day and Day-of-Week
