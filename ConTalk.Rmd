...
.pull-right[
```{r linear-cyclic, echo=FALSE, fig.height=7, fig.width = 5}
library(fpp2)
q1 <- autoplot(hsales) + xlab("Year") + ggtitle(" a) Linear arrangement of time") + ylab("")
...
p1 = SU_uniform_mutate %>% filter(Source=="B1 05") %>%
  ...
  ggplot(aes(x=wday, y=Value)) + geom_boxplot() + ylab("Energy consumption in KwH ") +  xlab("Day-of-Week") 
...
#   summarise(
#     Quantile = list(quantile(Value, c(0.1, 0.9, 0.1), na.rm = TRUE)),
#     Label = list(c(0.1, 0.9, 0.1))
#   ) %>%
#   as_tibble() %>%
#   unnest() %>%
#   as_tsibble(key = id(work, hour, Label), index = `Timestamp UTC`)
# 
# s1 %>% filter(Source=="B1 05") %>% group_by(work,hour) %>%
#   summarise(
...
ylab("Daily Consumption in KwH") +  xlab("Hour-of-Day")+ coord_cartesian(ylim = c(0,0.75)) + ggtitle("B1 05")
...
```{r linear-time, echo=FALSE, out.width="100%",out.height=240}
knitr::include_graphics("linear-time.png")
```
<br>
<br>
<br>
